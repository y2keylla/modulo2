...
app.post('/insereUsuario', urlencodedParser, (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	sql = "INSERT INTO dados_pessoais (nome, email, celular) VALUES ('" + req.body.nome + "', '" + req.body.email + "', " + req.body.celular + ")";
	console.log(sql);
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.post('/atualizaUsuario', (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
...
app.put('/atualizaUsuario', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = "UPDATE usuario SET nome='" + req.body.nome + "', email = '" + req.body.email + "' , celular='" + req.body.celular + "' WHERE id='" + req.body.id + "'";
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
...
app.get('/removedadospessoais', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = "DELETE FROM dados_pessoais WHERE id=" + req.query.id ;
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
